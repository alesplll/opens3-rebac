package tests

import (
	"context"
	"errors"
	"testing"

	authService "github.com/alesplll/opens3-rebac/services/auth/internal/service/auth"
	"github.com/alesplll/opens3-rebac/services/auth/pkg/mocks"
	"github.com/alesplll/opens3-rebac/shared/pkg/go-kit/logger"
	"github.com/alesplll/opens3-rebac/shared/pkg/go-kit/tokens"
	"github.com/brianvoe/gofakeit"
	"github.com/gojuno/minimock/v3"
	"github.com/stretchr/testify/require"
)

func TestGetRefreshToken(t *testing.T) {
	type args struct {
		ctx             context.Context
		oldRefreshToken string
	}

	logger.SetNopLogger()

	ctx := context.Background()
	oldRefreshToken := gofakeit.UUID()
	newRefreshToken := gofakeit.UUID()
	userID := gofakeit.UUID()
	email := gofakeit.Email()

	tokenErr := errors.New("token generation error")

	tests := []struct {
		name           string
		args           args
		wantToken      string
		wantErr        bool
		buildTokenMock func(t *testing.T, mc *minimock.Controller) *mocks.TokenServiceMock
	}{
		{
			name:      "success case",
			args:      args{ctx: ctx, oldRefreshToken: oldRefreshToken},
			wantToken: newRefreshToken,
			buildTokenMock: func(t *testing.T, mc *minimock.Controller) *mocks.TokenServiceMock {
				mock := mocks.NewTokenServiceMock(mc)
				mock.VerifyRefreshTokenMock.Expect(ctx, oldRefreshToken).Return(&tokens.UserClaims{
					UserId: userID,
					Email:  email,
				}, nil)
				mock.GenerateRefreshTokenMock.Return(newRefreshToken, nil)
				return mock
			},
		},
		{
			name:    "invalid refresh token",
			args:    args{ctx: ctx, oldRefreshToken: "invalid"},
			wantErr: true,
			buildTokenMock: func(t *testing.T, mc *minimock.Controller) *mocks.TokenServiceMock {
				mock := mocks.NewTokenServiceMock(mc)
				mock.VerifyRefreshTokenMock.Expect(ctx, "invalid").Return(nil, errors.New("invalid"))
				return mock
			},
		},
		{
			name:    "token generation error",
			args:    args{ctx: ctx, oldRefreshToken: oldRefreshToken},
			wantErr: true,
			buildTokenMock: func(t *testing.T, mc *minimock.Controller) *mocks.TokenServiceMock {
				mock := mocks.NewTokenServiceMock(mc)
				mock.VerifyRefreshTokenMock.Expect(ctx, oldRefreshToken).Return(&tokens.UserClaims{
					UserId: userID,
					Email:  email,
				}, nil)
				mock.GenerateRefreshTokenMock.Return("", tokenErr)
				return mock
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := minimock.NewController(t)

			tokenMock := tt.buildTokenMock(t, mc)
			userClientMock := mocks.NewUserClientMock(mc)
			repoMock := mocks.NewAuthRepositoryMock(mc)

			svc := authService.NewService(userClientMock, tokenMock, repoMock)

			token, err := svc.GetRefreshToken(tt.args.ctx, tt.args.oldRefreshToken)

			if tt.wantErr {
				require.Error(t, err)
				require.Empty(t, token)
				return
			}

			require.NoError(t, err)
			require.Equal(t, tt.wantToken, token)
		})
	}
}
