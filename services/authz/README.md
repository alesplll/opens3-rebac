...
### Свойства узлов и рёбер

`WriteTuple` проставляет:

| Свойство | Где | Смысл |
|---|---|---|
| `created_at` | узлы и рёбра | момент появления, миллисекунды |
| `updated_at` | рёбра | момент последней записи кортежа, миллисекунды |
| `actor` | рёбра | инициатор изменения из `WriteTupleRequest.actor`, если он передан |

Время проставляет сама база функцией `timestamp()`; сервис его не вычисляет и не
передаёт параметром. Поэтому метки не зависят от часов конкретного экземпляра
AuthZ и сравнимы между собой на всём графе. Повторная запись того же кортежа
сохраняет `created_at` и сдвигает `updated_at`.
...
`timestamp` в tuple event — это время, которым изменение записано в граф:
`write_tuple` и `delete_tuple` возвращают его из самого запроса, и сервис передаёт
это значение в событие. Поэтому ребро в Neo4j и событие в `auth-changes`,
описывающие одно изменение, несут одно время с одних часов. Payload решения
по-прежнему берёт время из часов экземпляра: у решения нет записи в графе.
